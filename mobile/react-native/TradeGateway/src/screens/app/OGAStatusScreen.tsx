/**
 * TradeGateway™ NGSWTP — React Native OGA Status Screen
 * Parity with PWA OGA Status page. Uses trpc.oga.list.useQuery.
 */
import React from "react";
import { View, Text, ScrollView, ActivityIndicator, StyleSheet, RefreshControl } from "react-native";
import { trpc } from "../../services/trpc";

export default function OGAStatusScreen() {
  const { data, isLoading, refetch } = trpc.oga.list.useQuery(undefined, { retry: 2 });
  ...
  return (
    ...
    >
      <Text style={styles.title}>OGA Status</Text>
      {isLoading ? (
