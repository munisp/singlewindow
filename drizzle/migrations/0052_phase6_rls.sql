-- Phase-6 SW-G3: RLS policies folded into the drizzle migration chain.
-- Source of truth was infra/postgres/01_rls_policies.sql (kept for reference);
-- this file makes the policies apply WITH the schema instead of out-of-band.
-- Idempotent: every CREATE POLICY is preceded by DROP POLICY IF EXISTS.
--
-- APP-ROLE REQUIREMENT: the runtime database role (DATABASE_URL) must NOT be
-- a superuser and must NOT have BYPASSRLS, or these policies are silently
-- ignored. Example provisioning:
--   CREATE ROLE ngswtp_app LOGIN PASSWORD '...' NOBYPASSRLS;
--   GRANT CONNECT ON DATABASE tradegateway TO ngswtp_app;
--   GRANT USAGE ON SCHEMA public TO ngswtp_app;
--   GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO ngswtp_app;
