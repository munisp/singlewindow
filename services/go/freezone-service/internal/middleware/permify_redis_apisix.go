...
// and APISIX admin API integration for freezone-service.
//
...
// APISIX:  Registers freezone-service routes via APISIX Admin API on startup.
//          Route: /api/v1/audit/* → freezone-service:8090
package middleware
...
func NewPermifyClient() *PermifyClient {
	...
	return &PermifyClient{
		...
		logger:     slog.Default().With("component", "permify", "service", "freezone-service"),
	}
}
...
func NewRedisClient() *RedisClient {
	...
	return &RedisClient{
		...
		logger:     slog.Default().With("component", "redis", "service", "freezone-service"),
	}
}
...
func NewAPISIXClient() *APISIXClient {
	...
	return &APISIXClient{
		...
		logger:     slog.Default().With("component", "apisix", "service", "freezone-service"),
	}
}

// RegisterRoutes registers freezone-service routes in APISIX on startup.
func (a *APISIXClient) RegisterRoutes(ctx context.Context, serviceHost string, servicePort int) error {
	routes := []map[string]interface{}{
		{
			"id":   "freezone-service-api",
