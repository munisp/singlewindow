...
// and APISIX admin API integration for asean-sw-service.
//
...
// APISIX:  Registers asean-sw-service routes via APISIX Admin API on startup.
//          Route: /api/v1/audit/* → asean-sw-service:8090
package middleware
...
func NewPermifyClient() *PermifyClient {
	...
	return &PermifyClient{
		...
		logger:     slog.Default().With("component", "permify", "service", "asean-sw-service"),
	}
}
...
func NewRedisClient() *RedisClient {
	...
	return &RedisClient{
		...
		logger:     slog.Default().With("component", "redis", "service", "asean-sw-service"),
	}
}
...
func NewAPISIXClient() *APISIXClient {
	...
	return &APISIXClient{
		...
		logger:     slog.Default().With("component", "apisix", "service", "asean-sw-service"),
	}
}

// RegisterRoutes registers asean-sw-service routes in APISIX on startup.
func (a *APISIXClient) RegisterRoutes(ctx context.Context, serviceHost string, servicePort int) error {
	routes := []map[string]interface{}{
		{
			"id":   "asean-sw-service-api",
			"name": "asean-sw-service",
			"uri":  "/api/v1/audit/*",
