"""
TradeGateway™ NGSWTP — Sanctions Screener Tests
...
def test_post_screen(client):
    """Test POST /screen endpoint."""
    if "POST" == "GET":
        response = client.get("/screen")
    else:
        response = client.post("/screen", json={"test": "data"})
...
def test_get_lists(client):
    """Test GET /lists endpoint."""
    if "GET" == "GET":
        response = client.get("/lists")
    else:
        response = client.post("/lists", json={"test": "data"})
    # Accept 200 (success), 422 (validation), 401 (auth required), 404 (not found)
    assert response.status_code in [200, 201, 422, 401, 404, 500]
