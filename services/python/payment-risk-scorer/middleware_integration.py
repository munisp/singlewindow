"""
TradeGateway™ NGSWTP — payment-risk-scorer Middleware Integration
...
def get_bundle() -> MiddlewareBundle:
    ...
    global _bundle
    if _bundle is None:
        _bundle = create_bundle("payment-risk-scorer")
    return _bundle
...
def setup_middleware() -> MiddlewareBundle:
    ...
    bundle = get_bundle()
    bundle.start()
    logger.info("[payment-risk-scorer] Full middleware bundle initialized — Kafka, Dapr, Keycloak, Permify, Redis, TigerBeetle, Lakehouse, APISIX")
    return bundle
...
def shutdown_middleware() -> None:
    ...
    if _bundle is not None:
        _bundle.stop()
        _bundle = None
        logger.info("[payment-risk-scorer] Middleware bundle shutdown complete")
...
def start_consumer_thread(topics: list[str], handler) -> None:
    ...
    logger.info(f"[payment-risk-scorer] Kafka consumer started for topics: {topics}")
