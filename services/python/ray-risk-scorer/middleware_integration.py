"""
Ray Risk Scorer — Middleware Integration
========================================
Wires Kafka, Dapr, Fluvio, and OpenTelemetry into the ray-risk-scorer service.

Ray distributed ML risk scoring service

Kafka topics consumed:
  declarations.submitted  → Distributed ML risk scoring using Ray cluster

Kafka topics published:
  ray.risk.scored

Fluvio streams:
  ray.risk.stream  → real-time feed
...
logger = logging.getLogger("ray-risk-scorer.middleware")

SERVICE_NAME = "ray-risk-scorer"
SERVICE_VERSION = "1.0.0"

TOPIC_DECLARATIONS_SUBMITTED = "declarations.submitted"
TOPIC_RAY_RISK_SCORED = "ray.risk.scored"
FLUVIO_RAY_RISK_STREAM = "ray.risk.stream"
...
def _handle_declarations_submitted(payload: dict[str, Any]) -> None:
    """
    Distributed ML risk scoring using Ray cluster
    """
    global mw
    if mw is None:
        return

    logger.info(f"[Middleware] Ray Risk Scorer handling {payload.get('declaration_id', payload.get('trader_id', 'unknown'))}")
    ...
    if mw:
        mw.publish_event(TOPIC_RAY_RISK_SCORED, result_event)

    if mw and mw.fluvio:
        mw.fluvio.produce(FLUVIO_RAY_RISK_STREAM, {
            "source": SERVICE_NAME,
...
def setup_middleware() -> None:
    """Initialise all middleware clients for ray-risk-scorer."""
